...
class _OrderViewState extends State<OrderView> {
    ...
    return PagedListView<int, MenuItem>(
      ...
      builderDelegate: PagedChildBuilderDelegate(
        ...
        Padding(
          ...
          child: MenuItemComponent(item: item, isGridView: false,)
        )
      )
    );
  }
  ...
  Widget buildGrid(){
    return PagedGridView<int, MenuItem>(
      pagingController: _pagination,
      gridDelegate: const SliverGridDelegateWithMaxCrossAxisExtent(maxCrossAxisExtent: 360.0, mainAxisExtent: 480.0),
      builderDelegate: PagedChildBuilderDelegate(
        itemBuilder: (context, item, index) =>
        Padding(
          padding: const EdgeInsets.all(GAP_SM),
          child: MenuItemComponent(item: item, isGridView: true,)
        ),
      )
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Material(
      ...
      child: isGridView ? buildGrid() : buildList(),
    );
  }
}
